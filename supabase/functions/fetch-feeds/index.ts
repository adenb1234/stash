...
function parseRss(channel: any, feedUrl: string) {
  ...
  for (const item of itemNodes) {
    ...
    items.push({
      ...
      title: itemTitle,
      excerpt,
      content: content.substring(0, 100000), // Keep HTML for links
      author,
      imageUrl,
      publishedAt,
    });
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "discover") {
      ...
      const basePaths = ['/feed', '/rss', '/feed.xml', '/rss.xml', '/atom.xml', '/index.xml', '/blog/feed', '/blog/rss', '/blog/feed.xml', '/blog/atom.xml', '/blog/index.xml', '/rss/feed.xml', '/posts.rss', '/articles/feed'];
      const baseUrl = new URL(url);

      // Helper to try a feed URL and return response if valid
      const tryFeedUrl = async (tryUrl: string) => {
      ...
      for (const path of basePaths) {
        try {
          const result = await tryFeedUrl(`${baseUrl.origin}${path}`);
      ...
      // Try path-relative feed discovery (e.g. /village/blog/feed)
      const urlPath = baseUrl.pathname.replace(/\/+$/, '');
      if (urlPath && urlPath !== '') {
        const feedSuffixes = ['/feed', '/rss', '/feed.xml', '/rss.xml', '/atom.xml', '/index.xml'];
        for (const suffix of feedSuffixes) {
          try {
            const result = await tryFeedUrl(`${baseUrl.origin}${urlPath}${suffix}`);
            if (result) return result;
          } catch (e) {
            // Try next suffix
          }
    ...
    if (action === "fetch_all") {
      ...
      // Auto-archive: delete unsaved feed items older than 100 days
      let archivedCount = 0;
      try {
        ...
        if (archivedCount > 0) {
          console.log(`Auto-archived ${archivedCount} old feed items`);
        }
      } catch (archiveErr) {
        console.error('Error archiving old feed items:', archiveErr.message);
      }
